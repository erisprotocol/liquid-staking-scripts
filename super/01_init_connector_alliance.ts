...
import { SuperInfoKeys, config } from "./config";
...
(async function () {
  ...
  addInfo("super", network, SuperInfoKeys.alliance_connector_addr(msg.gauge), addresses[0]);
  addInfo(
    "super",
    network,
    SuperInfoKeys.alliance_connector_zasset(msg.gauge),
    `factory/${addresses[0]}/${msg.zasset_denom}`
  );
  addInfo(
    "super",
    network,
    SuperInfoKeys.alliance_connector_vt(msg.gauge),
