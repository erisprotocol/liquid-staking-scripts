...
(async function () {
  ...
  const contract = getInfo("super", network, SuperInfoKeys.zapper_addr);
  ...
  const routes = [
    ...
    RouteBuilder.start(tokens.ampwhale).whale(tokens.whale).whale(tokens.luna).astro(tokens.axlUsdc),

    RouteBuilder.start(tokens.bonewhale).whale(tokens.whale).whale(tokens.luna).astro(tokens.axlUsdc),
